...
- Hot engine 0.2.0 follows a real-apply recovery: the renderer no longer duplicates a fixed wallpaper across `body`, the main surface, and the sidebar, and it no longer runs a renderer `MutationObserver` or timer that re-applied CSS on DOM updates. `disable` now accepts a pre-existing Inspector only after evaluating and matching the expected Codex main PID, then removes the renderer payload and closes that Inspector. A real recovery returned `true`, cleared local state, and left no port 9229 listener or scheduled task.
- Hot engine 0.2.1 adds a renderer health check and passed a no-restart dark-theme trial on Codex 26.715.7063.0. Apply returned `installed: true`; verification returned `healthy: true`, confirmed the XJTU style and body background, found `main.main-surface`, and reported `pointer-events: auto` for both body and main. Port 9229 was closed after both pulses. The dark preview remains active for user visual confirmation.
